...
class Product(Base):
    ...
    status: Mapped[ProductStatusEnum] = mapped_column(default=ProductStatusEnum.CREATED, server_default="CREATED")
...
class Category(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, server_default=text("gen_random_uuid()"))
    parent_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("catalog.categories.id", ondelete="SET NULL"))
