...
async def get_category_descendants(db: AsyncSession, category_id: uuid.UUID) -> list[uuid.UUID]:
...
async def get_products_list(
    ...
    if search:
        ...
        if len(search_val) >= 3:
            ...
            search_condition = (
                or_(Product.title.ilike(term, escape="/"), Product.description.ilike(term, escape="/"))
            )
