...
async def get_product_api(db: Annotated[AsyncSession, fastapi.Depends(db.get_db)], id: uuid.UUID) -> Product:
    try:
...
async def get_similar_product_api(
    ...
    try:
        return await product_service.get_similar_products(db, id, category, limit, offset)
    except ProductNotFoundError as err:
        raise fastapi.HTTPException(status_code=404, detail=str(err)) from err
