...
async def create_invoice_endpoint(
	invoice_data: InvoiceCreate, db: AsyncSession = Depends(get_db)
):
	try:
...
async def accept_invoice_endpoint(invoice_id: UUID, db: AsyncSession = Depends(get_db)):
	try:
		return await invoice_service.accept_invoice(db, invoice_id)
	except InvoiceNotFoundError as e:
		raise HTTPException(status_code=404, detail=str(e))
...
async def delete_invoice_endpoint(invoice_id: UUID, db: AsyncSession = Depends(get_db)):
	try:
